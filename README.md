...
If you don’t see startup logs or have HTTPS/dev-cert issues, run explicitly on HTTP:

```bash
ASPNETCORE_URLS=http://localhost:5038 ASPNETCORE_ENVIRONMENT=Development dotnet run --project src/API/API.csproj
```
